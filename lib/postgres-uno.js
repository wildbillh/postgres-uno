...
class PostgresUno extends EventEmitter{
     ...
     * @param {string | null} connectionConfig.string=null
     * @param {boolean} connectionConfig.encrypted=false
     * @param {boolean} connectionConfig.encryptedUser = false
    ...
    _connect(connectionConfig) {
        return new Promise ( (resolve, reject) => {
            ...
            let connection = Object.assign({}, connectionConfig);

            if (connection.encrypted) {
                connection.password = PostgresUno.decryptPass(connection.password)
            }
            if (connection.ecryptedUser) {
                connection.user = PostgresUno.decryptPass(connection.user)
            }
    ...
    /**
     * Used internally by the class to provide a simple decryption algorithm
     ...
     * @returns {Promise<object | string>} - On success, the promise will be resolved and a data object will be
     * returned. The promise will be rejected and an error message returned on error.
     * @returns {Promise<object | string>} results - The returned data object.
     ...
     * @returns {Array} results.fields - An Array of Objects describing the columns returned. Useful
